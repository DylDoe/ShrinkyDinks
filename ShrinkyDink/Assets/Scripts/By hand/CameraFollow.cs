...
public class CameraFollow : MonoBehaviour 
{
	...
	void LateUpdate()
	{
		...
		Vector2 focusPosition = focusArea.center + Vector2.up * verticalOffset; // For 2 players Add the vertical distance between the playesr divided by 2.

		if (focusArea.velocity.x != 0)
		{
			lookAheadDirX = Mathf.Sign(focusArea.velocity.x);
			if (Mathf.Sign(playerScript.directionalInput.x) == Mathf.Sign(focusArea.velocity.x) && playerScript.directionalInput.x != 0)
			{
