...
public class GroundMelee : MonoBehaviour {
	...
	void Update () {
		...
		tmr += Time.deltaTime;
		//check to see if should attack
		if(Mathf.Abs (this.transform.position.x - Target.x) + Mathf.Abs (this.transform.position.y - Target.y) <= AttackDistance){
			Attack();
		}
		...
		r = Physics2D.Raycast(this.transform.position, Vector2.down, 0.5f/*, this.GetComponent<SpriteRenderer>().bounds.size.y / 2 + 0.01f*/);
		if (r.collider == null) {
			return;
		} else {
			CurrentPlatform = r.collider.gameObject;
	...
	void HowToGetDown(){
		...
		if (CurrentPlatform.transform.position.x > PlayerChasing.transform.position.x) {
			...
			OffSet.x = CurrentPlatform.transform.position.x - ((CurrentPlatform.GetComponent<SpriteRenderer> ().bounds.size.x /2) + XJump);
		} else {
			//...if NO go right
			OffSet.y = CurrentPlatform.transform.position.y;
			OffSet.x = CurrentPlatform.transform.position.x + ((CurrentPlatform.GetComponent<SpriteRenderer> ().bounds.size.x /2) + XJump);
